...
SlurmOdysseyJobStringTemplateWithEmail="""#!/bin/bash
#SBATCH -p shared # Partition to submit to
#SBATCH -J Spots_Obs1
#SBATCH --job-name=%s 
#SBATCH --nodes=%s # Number of cores requested
#SBATCH --ntasks=%s
#SBATCH -N 1 # Ensure that all cores are on one machine

#SBATCH --output=%s # Standard out goes to this file
#SBATCH --error=%s # Standard err goes to this filehostname
#SBATCH --time=%s # runtime in minutes
#SBATCH --mem-per-cpu=%s  # Memory per cpu in MB (see also --mem-per-cpu)
#SBATCH --mail-user=%s
...
RAMDISK=/dev/shm/fbroekgaarden/ram
mkdir -p $RAMDISK
chmod 777 -R .
...
source activate BHNS # My environment is called 'BHNS'
...
SlurmOdysseyJobStringTemplateWithoutEmail="""#!/bin/bash
#SBATCH -p shared # Partition to submit to
#SBATCH -J Spots_Obs1
#SBATCH --job-name=%s 
#SBATCH --nodes=%s # Number of cores requested
...
def GenerateSlurmOdysseyJobString(job_name, number_of_nodes, number_of_cores, outfile, errfile, walltime, memory, send_email=False, user_email="", run_directory="./", command="./"):
	"""
	Parameters
	-----------
	job_name: str
	...
	"""
	if send_email:
		job_string = SlurmOdysseyJobStringTemplateWithEmail % (job_name, number_of_nodes, number_of_cores, outfile, errfile, walltime, memory, user_email, run_directory, command)
	else:
		job_string = SlurmOdysseyJobStringTemplateWithoutEmail % (job_name, number_of_nodes, number_of_cores, outfile, errfile, walltime, memory, run_directory, command)
