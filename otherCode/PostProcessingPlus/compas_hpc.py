...
AISRootDir = os.path.join(compasRootDir, "STROOPWAFELsampling/compasHPC")
...
AISCombineWalltime   = "100:00:00"
AISCombineMemory     = "50000"
...
if isAISRun:
	...
	AISExploratoryDir = os.path.join(rootOutputDir, "STROOPWAFELexploration")
	AISSamplingDir    = os.path.join(rootOutputDir, "STROOPWAFELrefinement")
	AISCombinedDir    = os.path.join(rootOutputDir, "STROOPWAFELcombined")
...
slurm_cluster_list  = ['ozstar', 'slurm', 'helios', 'odyssey']
...
if cluster in condor_cluster_list:
	...
	if(isAISRun):
		...
		for i in range(nBatches):
			...
			#-- Make a copy of the AIS step 3 script in each STROOPWAFELrefinement output directory
			bashCommand = 'cp ' + AISstep3Python + " " + sampling_dirName + '/.'
			chpc.runBashCommand(bashCommand, verbose=True)

			#-- Make a copy of the pythonSubmit in each STROOPWAFELrefinement output directory
	...
	if(isAISRun):
		...
		if cluster =='odyssey':
			job_string = chpc.GenerateSlurmOdysseyJobString(job_name, number_of_nodes, number_of_cores, outfile, errfile, walltime, memory, user_email=user_email, run_directory=run_directory, command=command)
	...
	if(isAISRun):
		...
		if cluster =='odyssey':
			ppString = chpc.GenerateSlurmOdysseyJobString(ppName, number_of_nodes, number_of_cores, ppOutputFile, ppErrorFile, ppWalltime, ppMemory, user_email=user_email, run_directory=AISExploratoryDir, command=ppCommand)
		...
		if cluster =='odyssey':
			ppString = chpc.GenerateSlurmOdysseyJobString(ppName, number_of_nodes, number_of_cores, ppOutputFile, ppErrorFile, ppWalltime, ppMemory, user_email=user_email, run_directory=rootOutputDir, command=ppCommand)
		else:
	...
	if(isAISRun):
		...
		if cluster =='odyssey':
			AISStep2SlurmString  = chpc.GenerateSlurmOdysseyJobString(AISstep2Name, number_of_nodes, number_of_cores, AISStep2OutputFile, AISStep2ErrorFile, ppWalltime, ppMemory, user_email=user_email, run_directory=newMasterFolderDir, command=AISstep2Command)
		else:
		...
		if cluster =='odyssey': 
			ppString = chpc.GenerateSlurmOdysseyJobString(ppName, number_of_nodes, number_of_cores, ppOutputFile, ppErrorFile, ppWalltime, ppMemory, user_email=user_email, run_directory=AISSamplingDir, command=ppCommand)
		else:
		...
		if cluster =='odyssey':
			AISUpdateWeightsSlurmString= chpc.GenerateSlurmOdysseyJobString(AISPostProcessingName, number_of_nodes, number_of_cores, AISUpdateWeightsOutputFile, AISUpdateWeightsErrorFile, ppWalltime, ppMemory, user_email=user_email, run_directory=newMasterFolderDir, command=AISUpdateWeightsCommand)
		else:
		...
		print(AISUpdateWeightsSlurmString)

		sbatchCommand = 'sbatch --dependency=afterok:' + str(pp2JobID)

		### ADDED 
		# Save AIS - post-processing job to a file
		sbatchAISPPFile = open(newMasterFolderDir+'/' + AISPostProcessingName +'.bash' ,'w')
		sbatchAISPPFile.write(AISUpdateWeightsSlurmString)
		sbatchAISPPFile.close()
		...
		if cluster =='odyssey':
			AISCombineString= chpc.GenerateSlurmOdysseyJobString(AISCombineName, number_of_nodes, number_of_cores, AISCombineOutputFile, AISCombineErrorFile, AISCombineWalltime, AISCombineMemory, send_email, user_email=user_email, run_directory=AISCombinedDir, command=AISCombineCommand)
		else:
		...
		sbatchAISCombineFile = open(AISCombinedDir+'/' + AISCombineName +'_Exploration_and_Refinement.bash' ,'w')
		sbatchAISCombineFile.write(AISCombineString)
		sbatchAISCombineFile.close()
	...
	#-- Copy master folder
	bashCommand = 'cp -r ' + masterFolderDir + ' ' + newMasterFolderDir
	print('copying ', 'cp -r ' + masterFolderDir + ' ' + newMasterFolderDir)
	print('------------')
	...
	if(isAISRun):
		...
		for i in range(nBatches):
			...
			exploratory_dirName = os.path.join(AISExploratoryDir, 'output' + str(i))
			exploratory_dirNames.append(str(exploratory_dirName))
			print('should have created output')
			print('cp -r ' + newMasterFolderDir + ' ' + exploratory_dirName)
			print()
			print()
			bashCommand = 'cp -r ' + newMasterFolderDir + ' ' + exploratory_dirName
			chpc.runBashCommand(bashCommand, verbose=True)
